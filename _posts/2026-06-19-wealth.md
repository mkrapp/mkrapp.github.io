...
Here are a few sliders to play around with.
...
And there are years (otherwise there wouldn't be much compounding, would there?)
...
I've also included *Uncertainty*. It increases or decreases the rates by that much, so we can check out worst and best case scenarios.
(The numbers in brackets show the values for worst- and best-case).

There is a line called *Retirement Income*.
This is an inflation-adjusted annual lump sum you would receive after tax, if you were to move all your wealth into a term deposit and just live off the interest rates.

The last two lines are sensitivities.
I talk about them [down below](#technical-details).
...
function run() {
  ...
  document.getElementById("results").innerHTML = `
    <tr><td>Nominal Capital <i>K<sub>n</sub></i></td><td>${fmtMoney(w.nominal)}</td><td>${fmtMoney(m.nominal)}</td><td>${fmtMoney(b.nominal)}</td></tr>
    <tr><td>Real Capital K<sub>r</sub></td><td>${fmtMoney(w.real)}</td><td>${fmtMoney(m.real)}</td><td>${fmtMoney(b.real)}</td></tr>
